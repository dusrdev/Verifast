namespace Verifast.Tests.Unit.Standard;

public class UserDto : IValidator<UserDto> {
    ...
    public void Validate(in UserDto instance, ref ValidationResult<string> result) {
        ...
        if (string.IsNullOrEmpty(instance.Password)) {
            ...
            if (!hasUpper) {
                result.AddError($"'{nameof(Password)}' must contain an uppercase letter");
            }

            if (!hasLower) {
                result.AddError($"'{nameof(Password)}' must contain a lowercase letter");
            }

            if (!hasDigit) {
                result.AddError($"'{nameof(Password)}' must contain a digit");
            }
        }
